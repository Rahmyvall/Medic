...
@section('tambah', 'Edit Pembayaran')
        ...
        <div class="row">
            <div class="col-12">
                <div class="card shadow-sm rounded">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('pembayaran.update', $pembayaran->pembayaran_id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('kunjungan_id') is-invalid @enderror" name="kunjungan_id"
                                    ...
                                    @foreach ($kunjungans as $k)
                                        <option value="{{ $k->id }}"
                                            {{ $pembayaran->kunjungan_id == $k->id ? 'selected' : '' }}>
                                            {{ $k->pasien->nama_pasien ?? '-' }} - {{ $k->dokter->nama_dokter ?? '-' }}
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01"
                                    ...
                                    id="total_tagihan" value="{{ old('total_tagihan', $pembayaran->total_tagihan) }}">
                                @error('total_tagihan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('metode_bayar') is-invalid @enderror" name="metode_bayar"
                                    ...
                                    <option value="cash"
                                        {{ old('metode_bayar', $pembayaran->metode_bayar) == 'cash' ? 'selected' : '' }}>
                                        Cash</option>
                                    <option value="BPJS"
                                        {{ old('metode_bayar', $pembayaran->metode_bayar) == 'BPJS' ? 'selected' : '' }}>
                                        BPJS</option>
                                    <option value="asuransi"
                                        {{ old('metode_bayar', $pembayaran->metode_bayar) == 'asuransi' ? 'selected' : '' }}>
                                        Asuransi</option>
                                </select>
                                @error('metode_bayar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('status') is-invalid @enderror" name="status"
                                    ...
                                    <option value="pending"
                                        {{ old('status', $pembayaran->status) == 'pending' ? 'selected' : '' }}>Pending
                                    </option>
                                    <option value="lunas"
                                        {{ old('status', $pembayaran->status) == 'lunas' ? 'selected' : '' }}>Lunas
                                    </option>
                                    <option value="dibatalkan"
                                        {{ old('status', $pembayaran->status) == 'dibatalkan' ? 'selected' : '' }}>
                                        Dibatalkan</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local"
                                    ...
                                    id="tanggal_bayar"
                                    value="{{ old('tanggal_bayar', optional($pembayaran->tanggal_bayar)->format('Y-m-d\TH:i')) }}">
                                @error('tanggal_bayar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
